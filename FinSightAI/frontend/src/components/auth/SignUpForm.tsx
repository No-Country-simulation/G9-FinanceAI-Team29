...
export default function SignUpForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!nombreLimpio || !apellidoLimpio || !emailLimpio || !password) {
    ...
    try {
      ...
      if (!data.user) {
        throw new Error(
          "Supabase no devolvió el usuario creado. Revisá la configuración de Auth.",
        );
      }
      ...
      if (data.session) {
        setMensaje(
          `Cuenta creada correctamente. Tu identificador es ${perfil.usuarioId}.`,
        );
        ...
        setMensaje(
          `Cuenta creada correctamente. Revisá ${emailLimpio} para confirmar tu correo antes de iniciar sesión.`,
        );
        ...
        <div>
          <div className="mb-5 sm:mb-8">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Registrate para comenzar a ver más allá de tus finanzas.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="space-y-5">
              ...
              {error && (
                <div
                  role="alert"
                  className="p-3 text-sm rounded-lg bg-error-50 text-error-600 dark:bg-error-500/10 dark:text-error-400"
                >
                  {error}
                </div>
              )}
              ...
              {mensaje && (
                <div
                  role="status"
                  className="p-3 text-sm text-green-700 bg-green-50 rounded-lg dark:bg-green-500/10 dark:text-green-400"
                >
                  {mensaje}
                </div>
              )}
