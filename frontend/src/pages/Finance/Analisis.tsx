...
export default function Analisis() {
  const [formData, setFormData] = useState<AnalisisRequest>({
    ...
    // Solo GASTOS y con valor > 0 (el backend rechaza montos negativos o cero).
    transacciones: [
      { descripcion: 'Alquiler departamento', valor: 1200 },
      { descripcion: 'Supermercado La Anonima', valor: 400 },
      { descripcion: 'Combustible YPF', valor: 150 },
      { descripcion: 'Netflix suscripcion', valor: 40 },
    ],
  });
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <span className="text-sm text-gray-500">
                    Confianza: {(resultado.probabilidad * 100).toFixed(1)}%
                  </span>
                  {resultado.motorAnalisis && (
                    <span className="text-xs text-gray-400 border border-gray-200 dark:border-gray-700 rounded px-2 py-0.5">
                      motor: {resultado.motorAnalisis}
                    </span>
                  )}
                </div>

                {typeof resultado.financialScore === 'number' && (
                  <div className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold text-gray-800 dark:text-white/90">
                        {resultado.financialScore}
                        <span className="text-base font-normal text-gray-400">/100</span>
                      </p>
                    </div>
                    {resultado.scoreStatus && (
                      <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium text-success-600 bg-success-50 dark:bg-success-500/10">
                        {resultado.scoreStatus}
                      </span>
                    )}
                  </div>
                ...
                <div className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                  ...
                  <p className={`text-lg font-bold ${
                    ...
                    (resultado.nivelRiesgo === 'Medio' || resultado.nivelRiesgo === 'Moderado') ? 'text-warning-600' : 'text-error-600'
                  }`}>
